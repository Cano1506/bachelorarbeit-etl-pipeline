...
# Erwartetes vollständiges Zeitfenster der September-Woche auf Stundenbasis.
...
def process_one_env_file(path) -> pd.DataFrame:
    """
    Liest eine einzelne Umwelt-JSON-Datei eines 12-Stunden-Blocks ein,
    ...
    park = [d for d in records if is_parkstrasse(d) and d.get("parameter") in PARAMS_PT10S_7]
...
def _reindex_to_full_week(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fehlende Stunden bleiben als NA erhalten und werden nicht künstlich aufgefüllt.
    """
...
def _print_na_report(df: pd.DataFrame):
    ...
    # Bericht nur für die in der Ausgabe erwarteten Fachspalten.
...
def main():
    """
    Lädt alle Umweltdateien der September-Woche,
    ...
    for d in daterange(d0, d1):
        # In der September-Lieferung liegt jeder Tag in zwei 12-Stunden-Blöcken vor.
        for suffix in ("00-12", "12-00"):
    ...
    out = PROCESSED_DIR / f"parkstrasse_umwelt_hourly_{WEEK_START}_bis_{WEEK_END}_sep2025.csv"
